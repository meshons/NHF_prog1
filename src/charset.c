...
void unicode_2_utf8(unsigned short *uni, char *utf8) {
    ...
    int pb, pk;
    pk = 0;
    for (pb = 0; uni[pb] != 0x0000; ++pb) {
        if (uni[pb] <= 0x007F)
        utf8[pk++] =uni[pb];
        else
            if (uni[pb] <= 0x07FF) {
                utf8[pk++] = 0xC0 | uni[pb] >> 6;
                utf8[pk++] = 0x80 | (uni[pb] & 0x3F);
            }
            else {
                utf8[pk++] = 0xE0 | uni[pb] >> 12;
                utf8[pk++] = 0x80 | ((uni[pb] >> 6) & 0x3F);
                utf8[pk++] = 0x80 | (uni[pb] & 0x3F);
            }
    }
...
void win1250_2_unicode(unsigned char *win1250,unsigned short *uni){
    unsigned short table[128] = {
    ...
    int i, j;
    j = 0;
    for (i = 0; win1250[i] != 0; ++i) {
        if (win1250[i] < 128)
            uni[j++] = win1250[i];
        else
            uni[j++] = table[win1250[i] - 128];
    }
    uni[j] = 0x0000;
}
